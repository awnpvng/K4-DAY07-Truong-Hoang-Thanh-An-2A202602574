...
class RecursiveChunker:
    ...
    def chunk(self, text: str) -> list[str]:
        if not text:
            return []
        return [part for part in self._split(text.strip(), self.separators) if part]
    ...
    def _split(self, current_text: str, remaining_separators: list[str]) -> list[str]:
        ...
        if separator == "":
            pieces = [current_text[index : index + self.chunk_size] for index in range(0, len(current_text), self.chunk_size)]
        else:
